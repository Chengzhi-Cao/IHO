...
class ParallEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # features[0]=[8,64,224,224]
        # features[1]=[8,128,112,112]
        # features[2]=[8,256,56,56]
        # features[3]=[8,512,28,28]
        # features[4]=[8,1024,14,14]
        # features[5]=[8,1024,7,7]
...
class Model(nn.Module):
    ...
    def forward(self, x):# x=[8,3,224,224]
        ...
        # encoder_skips[0]=[8,64,224,224]
        # encoder_skips[1]=[8,128,112,112]
        # encoder_skips[2]=[8,256,56,56]
        # encoder_skips[3]=[8,512,28,28]
        # encoder_skips[4]=[8,1024,14,14]
        # encoder_skips[5]=[8,1024,7,7]
        ...
        x_final = self.decoder_final(x4_up, None)# x_final=[8,64,224,224]
...
if __name__ == '__main__':
    ...
    nodes = 7
    channel_num = 2048
    inout = torch.randn((64,channel_num,nodes,nodes)).cuda() # inout=[1,2048,7,7]
    model = HyperEncoder_my(_node=nodes,channel=[channel_num]).cuda()
    ...
    # print('x2_up=',x2_up.shape)
    # print('x3_up=',x3_up.shape)
    # print('x4_up=',x4_up.shape)
    # print('logits=',logits.shape)
    # print('# generator parameters:', 1.0 * sum(param.numel() for param in model.parameters())/1000000)

    # model = HyperEncoder_my(_node=28)
    # inout = torch.randn((8,512,28,28))
