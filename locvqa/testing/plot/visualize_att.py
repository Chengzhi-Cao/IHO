...
def plot_attention_maps(model_name, model, visual, question, mask, answer, vocab_words, path_logs, question_indexes, vocab_answers):
    ...
    if model_name == 'VQA_MaskRegion':
        ...
        for i_s in range(g_out.shape[0]): # for every element of the batch
            ...
            f.tight_layout()
            ax[0].imshow(image)
            plt.imsave(jp(path_logs, 'att_maps', str(question_indexes[i_s].item()) + '_img' + '.png'),image)
            ax[0].axis('off')
            ...
            for i_glimpse in range(g_out.shape[1]): # for every glimpse
                ...
                plt.imsave(jp(path_logs, 'att_maps', str(question_indexes[i_s].item()) + '_{}'.format(i_glimpse) + '.png'),superimposed.astype(np.uint8))
        ...
        for i_s in range(g_out.shape[0]): # for every element of the batch
            ...
            # plt.imsave(jp(path_logs, 'att_maps', str(question_indexes[i_s].item()) + '_masked' + '.png'),ax[0])
