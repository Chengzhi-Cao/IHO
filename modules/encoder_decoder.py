...
class EncoderDecoder(AttModel):
    ...
    def __init__(self, args, tokenizer):
        ...
        self.d_model = args.d_model     # self.d_model=512
        self.d_ff = args.d_ff
        ...
        tgt_vocab = self.vocab_size + 1     # self.vocab_size=760
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        # # att_feats=[1,98,2048], att_masks=None, seq=[1,45]
        att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks, seq)  # att_feats=[1,98,512], att_masks=[1,1,98], seq=[1,44], seq_mask=[1,44,44]
        out = self.model(att_feats, seq, att_masks, seq_mask)   # out=[1,44,512]
        outputs = F.log_softmax(self.logit(out), dim=-1)    # outputs=[1,44,761]
        return outputs
